...
function Blah(path) {
    ...
    this.pack_path = ParentSearch(this.path, "package.json", { obj: true });
    if (this.pack_path) {
        this.blah_path = Path.join(pack_path.d, ".blah");
    } else {
        this.blah_path = null;
    }
}
...
Blah.prototype.init = function (callback) {
    var readme = new Streamp.Writable(Path.join(this.blah_path, "README.md"));
    Fs.createReadStream(__dirname + "/templates/README.md").pipe(readme).on("end", function () {
        callback();
    }).on("error", function (e) {
        callback(e);
    });
