...
function Blah(path) {
    ...
    // ~/.blah-templates
    this.usr_templates = Path.join(Ul.USER_DIR, BLAH_TEMPLATES);
...
Blah.prototype.docs = function (input, output, callback) {
    ...
    if (typeof output === "function") {
        callback = output;
        output = Path.join(self.path, DOCUMENTATION_FILE_NAME);
    }
...
Blah.prototype.readme = function (callback) {
    ...
    self.prepare(function (err) {
        ...
        SameTime([
            ...
            Ejs.renderFile(self.paths.readme, {
                docs: data[0]
              , _: self
              , require: require
