...
class Blah {
    ...
    render (path, data, callback) {
        ...
        this.search(DATA_JS, async (err, dataJsPath) => {
            let collectTemplateDataFn = async () => {}

            if (dataJsPath) {
                collectTemplateDataFn = require(dataJsPath)
            }

            const templateData = await collectTemplateDataFn(path, data, this)
            const renderData = ul.merge({
                _: this
              , require: require
            }, ul.merge(
                templateData,
