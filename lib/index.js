...
Blah.prototype.prepare = function (callback) {
    ...
    SameTime([
        function (cb) {
            var markdoxTmpl = Path.join(self.blah_dir, "markdox.ejs");
            IsThere(markdoxTmpl, function (exists) {
                self.markdox_tmpl = exists ? markdoxTmpl : __dirname + "/markdox-res/template.ejs";
                cb();
            });
        }
      , function (cb) {
...
Blah.prototype.docs = function (input, output, callback) {
    ...
    self.prepare(function (err) {
        ...
        MarkDox.process(input, {
          ...
          , output: output || DOCUMENTATION_FILE_NAME
        }, callback);
    });
};
//
