...
Blah.prototype.prepare = function (callback) {
    ...
    OneByOne([
      ...
      , function (next) {
            Exec("git log --pretty=format:%ad | tail -1 | awk -v N=5 '{print $N}'", function (err, stdout) {
                self.current_year = new Date().getFullYear().toString();
                self.first_commit_year = (err ? self.current_year : stdout.toString().trim()) || self.current_year;
                self.license_year = self.current_year === self.first_commit_year
                                  ? self.current_year : (self.first_commit_year + "-" + self.current_year.slice(-2))
                                  ;
                debugger
                next();
            });
...
Blah.prototype.license = function (license, callback) {
    ...
    OneByOne([
        self.prepare.bind(self)
      , function (next) {
            self.search("licenses/" + license + ".ejs", next);
        }
      , function (next, licensePath) {
            ...
            self.render(licensePath, next);
        }
      , function (next, content) {
            Fs.writeFile(
                Path.join(self.path, LICENSE_NAME)
              , content
              , next
            );
        }
